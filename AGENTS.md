...
4. **Schema discipline.** A schema change ships a `core/sql/migrations/NNN_*.sql` file, a regenerated `core/sql/schema.sql` and an updated `tests/fixtures/schema_baseline.sql` together; SQL lives only in `core/`; `player_id` is a constant; Chess960 exclusion goes through `core.chess.eligibility`.
