...
def test_listing_orders_contested_first_then_by_lines_then_by_fen(db: psycopg.Connection[DictRow]) -> None:
    ...
    h.line(db, 4, 1, "d", ["e4", "e5", "Bc4"])
    h.line(db, 5, 1, "e", ["e4", "e5", "Nf3"])
    rows = conflicts.listing(db)
    assert _fens(rows) == [AFTER_E5, AFTER_NC6]  # contested (two effective moves) before four latent lines
    assert rows[0]["contested"] and not rows[1]["contested"]
...
def test_a_chapter_coming_on_resolves_its_own_disagreement_by_plurality(db: psycopg.Connection[DictRow]) -> None:
    """Ruling 1: a toggle runs the import's rule, plurality included. Three lines of an off
...
def test_a_target_already_on_is_a_no_op_even_over_a_dirty_repertoire(db: psycopg.Connection[DictRow]) -> None:
    """Review r1: re-gating an active container would put its own effective lines through the
    cohort rule, switch the losers off and leave every result citing them stale."""
    h.book(db, 1, "B", "white")
    h.chapter(db, 1, 1, "C")
    h.line(db, 1, 1, "e4", ["e4", "e5"])
