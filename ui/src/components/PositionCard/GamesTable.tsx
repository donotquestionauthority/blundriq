...
export function GamesTable({ games, bestLabel }: { games: PositionCardGame[]; bestLabel: string }) {
  const hasMoves = games.some((g) => g.move_played || g.best_move);
  const hasCp = games.some((g) => g.cp_loss);
  return (
    <div>
      <p className="mb-2 text-xs uppercase tracking-wide text-zinc-500">Games ({games.length})</p>
